...
class ZoomUtils {
  ...
  static double validateScale(double scale, {double min = 0.1, double max = 10.0}) {
    assert(min > 0, 'Minimum scale must be greater than 0');
    assert(max > min, 'Maximum scale must be greater than minimum scale');
    
    if (scale < min) {
      if (kDebugMode) {
        print('Warning: Scale $scale is below minimum $min, clamping to minimum');
      }
      return min;
    }
    ...
    if (scale > max) {
      if (kDebugMode) {
        print('Warning: Scale $scale is above maximum $max, clamping to maximum');
      }
      return max;
    }
  ...
  static double validateDimension(double? dimension, double defaultValue) {
    ...
    if (dimension <= 0) {
      if (kDebugMode) {
        print('Warning: Invalid dimension $dimension, using default $defaultValue');
      }
      return defaultValue;
    }
